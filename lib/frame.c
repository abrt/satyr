 ...
 */
static const char *
findfirstabnul(const char *input, const char *a, const char *b,
...
int
btp_frame_parseadd_operator(const char **input, struct btp_strbuf *target)
{
    const char *local_input = *input;
...
int
btp_frame_parse_function_name_braces(const char **input, char **target)
{
    const char *local_input = *input;
    if (!btp_skip_char(&local_input, '('))
        return 0;
...
int
btp_frame_parse_function_name_template(const char **input, char **target)
{
    const char *local_input = *input;
    if (!btp_skip_char(&local_input, '<'))
        return 0;
...
bool
btp_frame_skip_function_args(const char **input, struct btp_location *location)
{
    const char *local_input = *input;
    if (!btp_skip_char(&local_input, '('))
    {
