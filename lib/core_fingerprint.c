...
static void
fingerprint_add_bool(struct btp_strbuf *buffer, const char *name, bool value)
{
...
static void
fp_has_cycle(struct btp_strbuf *buffer, GList *insns,
...
static void
fp_libcalls(struct btp_strbuf *buffer, GList *insns,
            uintptr_t begin, uintptr_t end, GHashTable *call_graph, GHashTable *plt)
{
    GList *it, *sym = NULL;
                              ...
                              char **error_message)
{
    ...
    while (thread)
    {
        ...
        while (frame)
        {
            ...
            {
                bool success = btp_core_fingerprint_generate_for_binary(thread,
                                                                        frame->file_name,
                                                                        error_message);
                if (!success)
                    return false;
...
compute_fingerprint(struct btp_core_frame *frame,
                    ...
                    struct btp_disassembler_state *disassembler,
                    char **error_message)
{
    struct btp_elf_frame_description_entry *fde = btp_elf_find_fde_for_address(eh_frame,
                                                                               frame->build_id_offset);
    if (!fde)
    {
        *error_message = btp_asprintf("No frame description entry found for an offset.");
        return false;
    }

    char **instructions = btp_disassembler_get_function_instructions(disassembler,
                                                                     fde->start_address,
                                                                     fde->length,
                                                                     error_message);
    if (!instructions)
    ...
    btp_disassembler_function_instructions_free(instructions);
    return true;
}
                                         ...
                                         char **error_message)
{
    ...
    struct btp_elf_plt_entry *plt = btp_elf_get_procedure_linkage_table(binary_path,
                                                                        error_message);
    if (!plt)
        return false;

    struct btp_elf_frame_description_entry *eh_frame = btp_elf_get_eh_frame(binary_path,
                                                                            error_message);
    if (!eh_frame)
    {
    ...
    struct btp_disassembler_state *disassembler = btp_disassembler_init(binary_path,
                                                                        error_message);
    if (!disassembler)
    {
        btp_elf_procedure_linkage_table_free(plt);
    ...
    while (thread)
    {
        ...
        while (frame)
        {
                ...
                0 == strcmp(frame->file_name, binary_path))
            {
                ...
                if (!success)
                {
                    btp_disassembler_free(disassembler);
                    btp_elf_procedure_linkage_table_free(plt);
                    btp_elf_eh_frame_free(eh_frame);
                    return false;
                }
